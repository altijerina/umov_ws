...
class SimpleLifecycleNode(Node):
    ...
    def on_shutdown(self, state: State) -> TransitionCallbackReturn:
        ...
        self.get_logger().info("Lifecycle Node on_shutdown() has been called.")
        return TransitionCallbackReturn.SUCCESS
    
    def on_cleanup(self, state: State) -> TransitionCallbackReturn:
        self.destroy_subscription(self.sub_)
        self.get_logger().info("Lifecycle Node on_cleanup() has been called.")
        return TransitionCallbackReturn.SUCCESS
    
    def on_activate(self, state: State) -> TransitionCallbackReturn:        
        self.get_logger().info("Lifecycle Node on_activate() has been called.")
        time.sleep(2)
        return super().on_activate(state)
    
    def on_deactivate(self, state: State) -> TransitionCallbackReturn:        
        self.get_logger().info("Lifecycle Node on_deactivate() has been called.")
