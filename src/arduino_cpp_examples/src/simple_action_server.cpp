...
public:
    ...
    : Node("simple_action_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<arduino_msgs::action::Fibonacci>(
...
private:
  rclcpp_action::Server<arduino_msgs::action::Fibonacci>::SharedPtr action_server_;

  rclcpp_action::GoalResponse goalCallback(
      const rclcpp_action::GoalUUID& uuid,
      std::shared_ptr<const arduino_msgs::action::Fibonacci::Goal> goal)
  {
  ...
  void acceptedCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduino_msgs::action::Fibonacci>> goal_handle)
  {
    // this needs to return quickly to avoid blocking the executor, so spin up a new thread
    std::thread{ std::bind(&SimpleActionServer::execute, this, _1), goal_handle }.detach();
  }

  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduino_msgs::action::Fibonacci>> goal_handle)
  {
    RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Executing goal");
    rclcpp::Rate loop_rate(1);
    const auto goal = goal_handle->get_goal();
    auto feedback = std::make_shared<arduino_msgs::action::Fibonacci::Feedback>();
    auto& sequence = feedback->partial_sequence;
    sequence.push_back(0);
    sequence.push_back(1);
    auto result = std::make_shared<arduino_msgs::action::Fibonacci::Result>();
...
RCLCPP_COMPONENTS_REGISTER_NODE(arduino_cpp_examples::SimpleActionServer)