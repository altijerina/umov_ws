...
std::vector<hardware_interface::CommandInterface> ArduinobotInterface::export_command_interfaces()
{
   ...
   {
        RCLCPP_INFO(rclcpp::get_logger("ArduinobotInterface"), "Deactivating the robot hardware.");
        if(arduino_.IsOpen());
        {
            try
            {
            ...
            {
                RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while closing connection with the port: " << port_);
                return CallbackReturn::FAILURE;
            }
